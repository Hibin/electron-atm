...
class StatesService{
  ...
  check_version(){
    if(this.settings){
      if (this.log)
        this.log.info('ATM Config ID: ' + this.settings.get('config_id'));
      ...
      try {
        if(settings_version && !semver.lt(settings_version, this.pkg_version)){
          ...
          this.settings.set('config_id', '0000');
          if(this.log){
            ...
            this.log.error('ATM Config ID reset to 0');
          }
        }
      } catch ( e ) {
  ...
  parseState(data){
    ...
    // parsed.set('number', parseInt(data.substring(0, 3), 10).toString().padStart(3, '0'));
    // parsed.set('number', parseInt(data.substring(0, 3), 36).toString().padStart(3, '0'));
    ...
    case 'K':
      {
        ...
        // state_to is used by State Navigator and must be Set
        let states_to = new Set();
